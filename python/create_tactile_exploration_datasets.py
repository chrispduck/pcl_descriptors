...
def split_and_save(model_name: str, obj_path: str, outdir: str, nsplits:int):
    ...
    [np.savetxt(fname=fname, X=arr, delimiter=',') for fname, arr in zip(fnames_out, splits)]
    # [ for fname, arr in zip(fnames_out, splits)]

if __name__ == '__main__':
    ap = argparse.ArgumentParser()
    ap.add_argument("--indir", type=str, default="../datasets/single/tactile/real/filtered/")
    ap.add_argument("--outdir",type=str, default="../datasets/tactile_exploration_1split_filtered_v2/")
    ...
    ap.add_argument("--cats", default=["baseball", "beer", "camera_box", "golf_ball", "orange", "pack_of_cards", "rubix_cube", "shampoo", "spam", "tape"])
    # ap.add_argument("--n-models", type=int, default=1)
    ...
    if args.n_models > 1:
        model_names = [cat + str(i) + ".csv" for cat in args.cats for i in range(args.n_models)]
    else:
